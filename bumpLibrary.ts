...
const libs: pathAndLibrary[] = [
  ...
  {path: "./h5p-bildetema-words-grid-view/src/library.ts", lib:h5pBildetemaWordsGridView},
  {path: "./h5p-bildetema-words-topic-image/src/library.ts", lib:h5pBildetemaWordsTopicImage},
  {path: "./h5p-bildetema-words-tree-view/src/library.ts", lib:h5pBildetemaWordsTreeView},
  {path: "./h5p-editor-bildetema-words-topic-image/src/library.ts", lib:h5pEditorBildetemaWordsTopicImage},
] 
...
libs.forEach((el) => {
  ...
  const bumpPatch = query.getYesNo(`Bump patchVersion for "${lib.title}" from ${currPatchVersion} -> ${bumpedPatchVersion}`)
  if(!bumpPatch) return
  ...
  fs.readFile(path, "utf8", (err, data) => {
    ...
    const libraryFileWithBumpedPatchVersion = dataArray.map(line => {
