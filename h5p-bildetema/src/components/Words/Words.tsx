...
export const Words: React.FC<WordsProps> = ({ words }) => {
    const [h5pContext, setH5PContext] = React.useState<IH5PContentType | undefined>(undefined);
    const ref = React.useRef<HTMLDivElement>(null);
    useEffect(() => {
        console.info("words", words);   
        console.info("h5p", H5P);
        const library = new (H5P as any).BildetemaWordsGridView({"bildetema-words-grid-view":words});
